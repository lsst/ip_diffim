...
#            diffIm2.writeFits('dA2')
        ...
        # OUTPUT
        if display:
            ds9.mtv(kImageOut, frame=5+foffset)
            ds9.mtv(diffIm2, frame=6+foffset)
        if writefits:
    ...
    def testFunctor(self):
        ...
        for object in self.footprints:
            ...
            dKernel = self.applyFunctor(invert=True,  foffset=6, 
                                        xloc= int(0.5 * ( object.getBBox().getX0() + object.getBBox().getX1() )),
                                        yloc= int(0.5 * ( object.getBBox().getY0() + object.getBBox().getY1() )))
        ...
        cMean, cU, cVal, cCoeff = ipDiffim.runPca(cM, None)
        dMean, dU, dVal, dCoeff = ipDiffim.runPca(dM, None)
