...
class AlardLuptonSubtractTest(lsst.utils.tests.TestCase):
    ...
    def test_calibrated_images(self):
        """Check that the images subtract properly and the difference image is
        calibrated when the template and science images have non-trivial
        ...
        def _run_and_check_images(task, sciScale, templScale):
            """Create test images with non-trivial photometric calibrations.
            """
            science, sources = makeTestImage(psfSize=3.0, noiseLevel=noiseLevel, noiseSeed=6,
                                             calibration=sciScale)
            template, _ = makeTestImage(psfSize=2.0, noiseLevel=noiseLevel, noiseSeed=7,
                                        templateBorderSize=20,
                                        calibration=templScale, doApplyCalibration=True)
            ...
            self.assertFloatsAlmostEqual(np.mean(output.difference.image.array), 0, atol=5*mean_error)
            # stddev of difference image should be close to expected value.
            self.assertFloatsAlmostEqual(np.std(output.difference.image.array),
                                         np.sqrt(2)*noiseLevel, rtol=0.1)

        # Make test images with different calibration scalings for the science and template
        _run_and_check_images(task, sciScale=0.3, templScale=0.3)
        _run_and_check_images(task, sciScale=0.3, templScale=4.2)
        _run_and_check_images(task, sciScale=4.2, templScale=0.3)
        _run_and_check_images(task, sciScale=4.2, templScale=4.2)

    def test_miscalibrated_images(self):
        """Check that the images still subtract properly and the difference
        image is miscalibrated by an expected amount if the template and
        science images have incorrect photometric calibrations.
        """
        noiseLevel = 1.
        config = subtractImages.AlardLuptonSubtractTask.ConfigClass()
        config.doSubtractBackground = False
        ...
        def _run_and_check_images(task, sciScale, templScale):
            """Create test images with non-trivial and incorrect photometric calibrations.
            """
            science, sources = makeTestImage(psfSize=3.0, noiseLevel=noiseLevel, noiseSeed=6,
                                             calibration=sciScale, doApplyCalibration=True)
            template, _ = makeTestImage(psfSize=2.0, noiseLevel=noiseLevel, noiseSeed=7,
                                        templateBorderSize=20,
                                        calibration=templScale)
            # import ipdb; ipdb.set_trace()
            output = task.run(template, science, sources)
            # There should be no NaNs in the image if we convolve the template with a buffer
            self.assertTrue(np.all(np.isfinite(output.difference.image.array)))
            # Mean of difference image should be close to zero.
            mean_error = noiseLevel*sciScale/np.sqrt(output.difference.image.array.size)
            self.assertFloatsAlmostEqual(np.mean(output.difference.image.array), 0, atol=5*mean_error)
            # stddev of difference image should be close to expected value.
            self.assertFloatsAlmostEqual(np.std(output.difference.image.array),
                                         np.sqrt(2)*noiseLevel*sciScale, rtol=0.1)

        # Make test images with different *incorrect* calibration scalings for the science and template
        _run_and_check_images(task, sciScale=0.3, templScale=0.3)
        _run_and_check_images(task, sciScale=0.3, templScale=4.2)
        _run_and_check_images(task, sciScale=4.2, templScale=0.3)
        _run_and_check_images(task, sciScale=4.2, templScale=4.2)
