...
def fitSpatialFunction(spatialFunction, values, variances, col, row, policy):
    ...
    parameters    = afwMath.Vector(nParameters, 0.0)  # start with no spatial variation
    parameters[0] = 1.0                               # except for the constant term
    stepsize      = atfMath.Vector(nParameters, stepsize)
...
def fitPerPixel(differenceImageFootprintInformationList, policy):
    ...
    footprintExposureCol  = afwMath.Vector(nFootprint, 0.0)
    footprintExposureRow  = afwMath.Vector(nFootprint, 0.0)
    footprintVariances    = afwMath.Vector(nFootprint, 0.0)
    ...
    for kCol in range(kernelCols):
        for kRow in range(kernelRows):
            ...
            kernelValues = afwMath.Vector(nFootprint, 0.0)
            for i in range(nFootprint):
    ...
    # fit the background
    backgroundValues    = afwMath.Vector(nFootprint, 0.0)
    for i in range(nFootprint):
...
def fitPca(differenceImageFootprintInformationList, policy):
    ...
    # matrix to invert
    M = afwMath.Matrix(kernelCols*kernelRows, nFootprint)
    for i in range(nFootprint):
    ...
    # do the PCA
    meanM = afwMath.Vector(kernelCols*kernelRows, 0.0)
    eVal  = afwMath.Vector(nFootprint, 0.0)
    eVec  = afwMath.Matrix(kernelCols*kernelRows, nFootprint, 0.0)
    ...
    eCoefficients = afwMath.Matrix(nCoefficients, nFootprint, 0.0)
    ...
    for i in range(nFootprint):
        ...
        for nCoeff in range(nCoefficients):
            eCoefficient              = afwMath.dot(singleKernelVector, eVec[nCoeff,:])
...
""" % (defSciencePath, defTemplatePath, defOutputPath, defPolicyPath)
    ...
    kernelBasisList = diffimLib.generateDeltaFunctionKernelSet(kernelCols,
                                                               kernelRows)
    ...
    convolveDifiList   = diffimLib.vectorDiffImContainerD()
    deconvolveDifiList = diffimLib.vectorDiffImContainerD()
    for footprintID, iFootprintPtr in enumerate(footprintList):
        ...
        convKernelCoeffList, convBackground = diffimLib.computePsfMatchingKernelForPostageStamp(
        ...
        deconvKernelCoeffList, deconvBackground = diffimLib.computePsfMatchingKernelForPostageStamp(
            imageStampPtr.get(),
            templateStampPtr.get(),
            kernelBasisList,
            policy,
        ...
        convDifi   = diffimLib.DiffImContainerD()
        ...
        deconvDifi = diffimLib.DiffImContainerD()
