    ...
    */
    ...
    if (cswitch == 2) {
        ...
        for (unsigned int kidxi = 0; eiteri != eiterE; eiteri++, kidxi++) {
            //Eigen::Cwise<Eigen::CwiseBinaryOp<Eigen::ei_scalar_product_op<double>, Eigen::VectorXd, Eigen::VectorXd> >
            //eiteriDotiVariance = ((*eiteri)->cwise() * eigeniVarianceV).cwise();
