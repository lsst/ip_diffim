...
from lsst.ip.diffim.checkTemplateQuality import CheckTemplateQualityTask
...
class GetTemplateConfig(
    ...
    checkTemplateQuality = pexConfig.ConfigurableField(
        target=CheckTemplateQualityTask,
        doc="Task to set mask planes flagging template regions (high variance,"
        " shallow coverage, PSF discontinuities) that would degrade differencing.",
    )
...
class GetTemplateTask(pipeBase.PipelineTask):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.makeSubtask("checkTemplateQuality")
    ...
    def run(self, *, coaddExposureHandles, bbox, wcs, dataIds, physical_filter, visit=None):
        ...
        warped = {}
        catalogs = []
        # Per-tract inputs for the template-quality checks, captured in the loop
        # because the final merge mutates the warped masks in place. ``hasData``
        # marks where each tract contributes; the per-CCD inputs give its depth.
        qualityConfig = self.config.checkTemplateQuality
        captureInputs = qualityConfig.doMaskShallowCoverage or qualityConfig.doMaskPsfDiscontinuity
        hasDataByTract = {}
        ...
        for tract in coaddExposureHandles:
            ...
            # Capture per-tract masking inputs from the pristine warped exposure,
            # before the final merge below mutates these masks in place.
            if captureInputs:
                ccdInputCatalogsByTract[tract] = tractCcdInputs
            if qualityConfig.doMaskShallowCoverage:
                hasDataByTract[tract] = (
                    (potentialInput.mask.array
                     & potentialInput.mask.getPlaneBitMask("NO_DATA")) == 0
                )
        ...
        # Set template-quality mask planes (high variance, shallow coverage, and
        # PSF discontinuities) that exclude unreliable regions downstream.
        self.checkTemplateQuality.run(template, hasDataByTract, ccdInputCatalogsByTract)
    ...
    def _makeExposureCatalog(self, exposureRefs, dataIds):
        ...
        for coadd, dataId in zip(exposures, dataIds):
            ...
            if (self.config.checkTemplateQuality.doMaskShallowCoverage
                    or self.config.checkTemplateQuality.doMaskPsfDiscontinuity):
