...
class MaskReferenceSourcesTask(pipeBase.PipelineTask):
    ...
    def run(self, ref_catalog, difference_image, matching_sources, matching_image):
        ...
        """
        self.log.info("Masking reference sources in difference image")
        ...
        if len(matches) == 0:
            self.log.warn("No reference sources matched to detected sources")
    ...
    def _matchSources(self, ref_catalog, matching_sources, wcs):
        ...
        self.log.debug(f"Matching: {len(matches)} matched, {n_too_far}"
                       f" beyond {self.config.matching_radius} arcsec")
        return matches
