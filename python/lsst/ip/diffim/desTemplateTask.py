...
class DesTemplateTask(pipeBase.PipelineTask):
    ...
    def run(self, bbox, wcs, band, physical_filter):
        ...
        required_columns = ['tilename', 'band', 'survey', 'ra_cent', 'dec_cent', 'rac1', 'rac2', 'rac3', 'rac4', 
                           'decc1', 'decc2', 'decc3', 'decc4', 'filepath']
        missing_columns = [col for col in required_columns if col not in tile_table.colnames]
        if missing_columns:
            raise RuntimeError(f"Required columns missing from tile table: {missing_columns}. "
        ...
        if not overlapping_tiles:
            ...
            self.log.info(f"Target was centered at RA={target_ra:.3f}, Dec={target_dec:.3f}")
        ...
        resolved_tiles = self._resolveDuplicateTiles(overlapping_tiles, band_filtered_tiles)
    ...
    def _findOverlappingTiles(self, bbox, wcs, tile_table):
        ...
        for row in tile_table:
            ...
            required_corner_columns = [
                "rac1", "decc1", "rac2", "decc2", 
                "rac3", "decc3", "rac4", "decc4"
            ]
            
            if not all(col in tile_table.colnames for col in required_corner_columns):
                missing_corners = [col for col in required_corner_columns if col not in tile_table.colnames]
                raise RuntimeError(f"Required corner coordinate columns missing from tile table: {missing_corners}")
            ...
            if has_overlap:
                ...
                distance = np.sqrt(
                    (tile_ra - target_ra) ** 2 + (tile_dec - target_dec) ** 2
                )

                self.log.info(
                    f"  OVERLAP: {tile_name} - distance={distance:.3f}°"
                )
    ...
    def _filterTilesByBand(self, tile_table, band):
        ...
        if 'band' not in tile_table.colnames:
            raise RuntimeError("Required 'band' column not found in tile table. "
                             "Please use the merged tiles CSV format.")
        
        if 'survey' not in tile_table.colnames:
            raise RuntimeError("Required 'survey' column not found in tile table. "
                             "Please use the merged tiles CSV format.")
        ...
        if invalid_count > 0:
            self.log.warn(f"Found {invalid_count} tiles with unrecognized survey values, ignoring them")
        
        filtered_table = band_filtered[valid_mask]
        
        self.log.info(f"Filtered {len(tile_table)} tiles to {len(filtered_table)} for band '{band}' with valid surveys")
    ...
    def _resolveDuplicateTiles(self, overlapping_tiles, band_filtered_tiles):
        ...
        for tile_name, distance in overlapping_tiles:
            ...
            if len(tile_entries) == 0:
                ...
                for preferred_survey in priority_order:
                    survey_entries = tile_entries[tile_entries['survey'] == preferred_survey]
                    if len(survey_entries) > 0:
                        selected_entry = survey_entries[0]
                        self.log.info(f"Using {preferred_survey} tile for {tile_name} (priority)")
                        break
                
                if selected_entry is None:
                    # No recognized survey, use first available
                    selected_entry = tile_entries[0]
                    self.log.info(f"Using {selected_entry['survey']} tile for {tile_name} (fallback)")
        ...
        self.log.info(f"Resolved {len(resolved_tiles)} tiles from {len(overlapping_tiles)} overlapping tiles")
